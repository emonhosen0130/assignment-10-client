...
import PaintingDrawingCard from '../Cards/PaintingDrawingCard';
import ManyCard from "../Cards/ManyCard";
        ...
        <div className="">
            ...
            <div className="gadgetContainer">
                ...
                <div className=" grid grid-cols-2 md:grid-cols-4 sm:grid-cols-3  xl:grid-cols-5 gap-5  mt-10 w-full ">
                    ...
                    {brands?.map((brand) => (
                        <PaintingDrawingCard key={brand.id} brand={brand} />
                    ))}
                </div>
            </div>
            {/* Explore our product */}
            <div className="gadgetContainer">
                ...
                <div className="grid xl:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1  gap-4 mt-10">
                    {allProducts?.length > 8
                        ? allProducts
                            ?.slice(0, 8)
                            .map((item) => <ManyCard key={item._id} item={item} />)
                        : allProducts?.map((item) => (
                            <ManyCard key={item._id} item={item} />
                        ))}
                </div>
                {allProducts?.length > 8 && (
                        ...
                        <div className="flex flex-wrap -m-4">
                            <div className="p-4 md:w-1/2 w-full ">
                                <div className="h-full bg-gray-100 dark:bg-[#1a2641d5] p-8 shadow-lg rounded">
                                    ...
                                    <p className="leading-relaxed mb-6">I recently purchased the Apple MacBook Air, and I must say, Im thoroughly impressed with this fantastic piece of technology. From its design to its performance, it has exceeded my expectations in every way.</p>
